...
import { BerkasKlaimClinicalLabClaimEndpoint, useClinicalLabClaimResource } from '../api'
import type { ClinicalLabClaim } from '../types'

const columns: ColumnDef<ClinicalLabClaim, unknown>[] = [
  {
...
const actions: WorkflowAction<ClinicalLabClaim>[] = []

export function ClinicalLabClaimListPage() {
  const resource = useClinicalLabClaimResource()
  const title = humanizeModuleName('BerkasKlaimClinicalLabClaim')

  return (
    <WorkflowListPage<ClinicalLabClaim>
      title={title}
      description={`Kelola data ${title.toLowerCase()}.`}
      endpoint={BerkasKlaimClinicalLabClaimEndpoint}
      columns={columns}
