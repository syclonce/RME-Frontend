...
import { useForearmExaminationResource } from '../api'
import type { ForearmExaminationFormValues } from '../types'

export function ForearmExaminationFormPage() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const isEdit = id !== undefined
  const { create, update, useDetail } = useForearmExaminationResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<ForearmExaminationFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as ForearmExaminationFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-forearm-examination') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-forearm-examination') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} ForearmExamination</h1>
