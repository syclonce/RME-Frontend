...
import type { HandJointExaminationFormValues } from '../types'

export function HandJointExaminationFormPage() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const isEdit = id !== undefined
  const { create, update, useDetail } = useHandJointExaminationResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<HandJointExaminationFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as HandJointExaminationFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-hand-joint-examination') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-hand-joint-examination') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} HandJointExamination</h1>
      ...
      <div className="grid gap-1.5">
        ...
        <Select value={values.joint ?? ''} onValueChange={(v) => setValues({ ...values, joint: v })}>
          ...
          <SelectContent>
            <SelectItem key="wrist" value="wrist">Wrist</SelectItem>
            <SelectItem key="finger" value="finger">Finger</SelectItem>
            <SelectItem key="elbow" value="elbow">Elbow</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <div className="grid gap-1.5">
