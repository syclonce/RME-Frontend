...
export function PharmacyServiceFeeFormPage() {
  ...
  const { create, update, useDetail } = usePharmacyServiceFeeResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<PharmacyServiceFeeFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as PharmacyServiceFeeFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/layanan-pharmacy-service-fee') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/layanan-pharmacy-service-fee') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} PharmacyServiceFee</h1>
