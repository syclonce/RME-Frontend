...
export function DepositFormPage() {
  ...
  const { create, update, useDetail } = useDepositResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<DepositFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as DepositFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/pembayaran-deposit') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/pembayaran-deposit') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} Deposit</h1>
