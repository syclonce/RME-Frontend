...
export function NursingCarePlanImplementationFormPage() {
  ...
  const { create, update, useDetail } = useNursingCarePlanImplementationResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<NursingCarePlanImplementationFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as NursingCarePlanImplementationFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-nursing-care-plan-implementation') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-nursing-care-plan-implementation') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} NursingCarePlanImplementation</h1>
      <div className="grid gap-1.5">
        <Label htmlFor="nursing_care_plan_id">Nursing Care Plan *</Label>
        <RelationSelect
          endpoint="/nursing-care-plans"
          value={values.nursing_care_plan_id ?? null}
          onChange={(v) => setValues({ ...values, nursing_care_plan_id: v })}
        />
      </div>
      <div className="grid gap-1.5">
