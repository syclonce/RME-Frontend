...
export function DiscountFormPage() {
  ...
  const { create, update, useDetail } = useDiscountResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<DiscountFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as DiscountFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/pembayaran-discount') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/pembayaran-discount') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} Discount</h1>
