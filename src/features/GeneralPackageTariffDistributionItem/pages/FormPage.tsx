...
export function PackageTariffDistributionItemFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as PackageTariffDistributionItemFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/general-package-tariff-distribution-item') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/general-package-tariff-distribution-item') })
      }}
    >
