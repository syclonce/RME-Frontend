...
export function VideoAttachmentFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as VideoAttachmentFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/general-video-attachment') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/general-video-attachment') })
      }}
    >
