...
export function AdmissionDiagnosisFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as AdmissionDiagnosisFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/general-admission-diagnosis') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/general-admission-diagnosis') })
      }}
    >
