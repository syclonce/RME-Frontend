...
import { MedicalRecordTransferMedicationReconciliationItemEndpoint, useTransferMedicationReconciliationItemResource } from '../api'
import type { TransferMedicationReconciliationItem } from '../types'

const columns: ColumnDef<TransferMedicationReconciliationItem, unknown>[] = [
  {
    header: humanizeField('reconciliation_id'),
    cell: ({ row }) => <RelationLabel endpoint="/transfer-med-reconciliations" id={(row.original as unknown as Record<string, unknown>).reconciliation_id as number | null} />,
  },
  {
...
const fields: CrudField[] = [
  { key: 'reconciliation_id', label: humanizeField('reconciliation_id'), type: 'relation', relationEndpoint: '/transfer-med-reconciliations', required: true },
...
const actions: WorkflowAction<TransferMedicationReconciliationItem>[] = []

export function TransferMedicationReconciliationItemListPage() {
  const resource = useTransferMedicationReconciliationItemResource()
  const title = humanizeModuleName('MedicalRecordTransferMedicationReconciliationItem')

  return (
    <WorkflowListPage<TransferMedicationReconciliationItem>
      title={title}
      description={`Kelola data ${title.toLowerCase()}.`}
      endpoint={MedicalRecordTransferMedicationReconciliationItemEndpoint}
      columns={columns}
