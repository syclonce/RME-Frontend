...
export function KfaProductsListPage() {
  const { data, isLoading } = useQuery({
    queryKey: ['/kfa/products', 'list'],
    queryFn: async () => {
      const res = await apiClient.get('/kfa/products')
  ...
  return (
    <div className="p-4">
      <h1 className="mb-4 text-lg font-semibold">KfaProducts (read-only)</h1>
      <Table>
        <TableHeader>
          <TableRow>
            {COLUMNS.map((col) => (
