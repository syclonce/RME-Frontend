...
export function PrescriptionFrequencyRuleFormPage() {
  ...
  const { create, update, useDetail } = usePrescriptionFrequencyRuleResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<PrescriptionFrequencyRuleFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as PrescriptionFrequencyRuleFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/general-prescription-frequency-rule') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/general-prescription-frequency-rule') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} PrescriptionFrequencyRule</h1>
