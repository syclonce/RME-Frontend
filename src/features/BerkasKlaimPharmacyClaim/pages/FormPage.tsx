...
export function PharmacyClaimFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as PharmacyClaimFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/berkas-klaim-pharmacy-claim') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/berkas-klaim-pharmacy-claim') })
      }}
    >
