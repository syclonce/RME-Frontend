...
export function InventoryPharmacyPackageFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as InventoryPharmacyPackageFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/inventory-pharmacy-package') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/inventory-pharmacy-package') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} InventoryPharmacyPackage</h1>
