...
export function DiagnosisIndicatorMappingFormPage() {
  ...
  const { create, update, useDetail } = useDiagnosisIndicatorMappingResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<DiagnosisIndicatorMappingFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as DiagnosisIndicatorMappingFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-diagnosis-indicator-mapping') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-diagnosis-indicator-mapping') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} DiagnosisIndicatorMapping</h1>
