...
export function PackageFormPage() {
  ...
  const { create, update, useDetail } = usePackageResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<PackageFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as PackageFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/general-package') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/general-package') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} Package</h1>
