...
export function ServiceFormPage() {
  ...
  const { create, update, useDetail } = useServiceResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<ServiceFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as ServiceFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/general-service') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/general-service') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} Service</h1>
