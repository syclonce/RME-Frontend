...
export function Icd9CmCodeFormPage() {
  ...
  const { create, update, useDetail } = useIcd9CmCodeResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<Icd9CmCodeFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as Icd9CmCodeFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-icd9-cm-code') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-icd9-cm-code') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} Icd9CmCode</h1>
