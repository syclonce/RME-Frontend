...
export function MedicalSupplyUsageFormPage() {
  ...
  const { create, update, useDetail } = useMedicalSupplyUsageResource()
  const existing = useDetail(isEdit ? Number(id) : undefined)
  const [values, setValues] = useState<MedicalSupplyUsageFormValues>({})

  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as MedicalSupplyUsageFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/layanan-medical-supply-usage') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/layanan-medical-supply-usage') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} MedicalSupplyUsage</h1>
