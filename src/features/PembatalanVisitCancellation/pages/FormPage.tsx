...
export function VisitCancellationFormPage() {
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        create.mutate(values, { onSuccess: () => navigate('/modul/pembatalan-visit-cancellation') })
      }}
    >
      ...
      <div className="grid gap-1.5">
        <Label htmlFor="cancelled_by">Cancelled By *</Label>
        <Input id="cancelled_by" type="number" value={values.cancelled_by ?? ''} onChange={(e) => setValues({ ...values, cancelled_by: e.target.value === '' ? null : Number(e.target.value) })} />
      </div>
      <div className="grid gap-1.5">
