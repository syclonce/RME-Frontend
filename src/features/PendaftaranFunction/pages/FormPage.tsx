...
import type { PendaftaranFunctionFormValues } from '../types'

export function PendaftaranFunctionFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as PendaftaranFunctionFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/pendaftaran-function') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/pendaftaran-function') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} PendaftaranFunction</h1>
