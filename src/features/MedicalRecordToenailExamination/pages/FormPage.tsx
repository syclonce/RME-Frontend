...
export function ToenailExaminationFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as ToenailExaminationFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-toenail-examination') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-toenail-examination') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} ToenailExamination</h1>
