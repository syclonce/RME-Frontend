...
export function NursingIndicatorFormPage() {
  ...
  useEffect(() => {
    if (existing.data) setValues(existing.data as unknown as NursingIndicatorFormValues)
  }, [existing.data])
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault()
        if (isEdit) update.mutate({ id: Number(id), payload: values }, { onSuccess: () => navigate('/modul/medical-record-nursing-indicator') })
        else create.mutate(values, { onSuccess: () => navigate('/modul/medical-record-nursing-indicator') })
      }}
    >
      <h1 className="text-lg font-semibold">{isEdit ? 'Ubah' : 'Tambah'} NursingIndicator</h1>
      ...
      <div className="grid gap-1.5">
        ...
        <Input id="target_value" type="text" value={values.target_value ?? ''} onChange={(e) => setValues({ ...values, target_value: e.target.value })} />
      </div>
